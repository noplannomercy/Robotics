...
## 🟢 P0 — MVP 현상 파악 (지금 이것만). 설계: `2026-06-16-역문서-mvp-현상파악-design.md`

- [ ] **M1. 의존성 순서 정렬기** — 선별된 의존성 데이터(DBA_DEPENDENCIES, `03_Phase1_데이터준비`)를 위상 정렬 → 순서 큐(DDL/딕셔너리 → 말단 → 상위)
- [ ] **M2. 순차 적재 드라이버** — 순서 큐대로 패키지 바디를 기존 `to_reverse_doc` 파이프라인에 투입 (신규 코드 최소)
- [ ] **M3. 현상 관찰 기록** — 역문서 샘플 품질·빈약 지점(특히 caller)·기준1·2 통과율 1건 문서화

> 검증은 이미 있는 Inner 루프 **기준 1·2만** 사용(오늘 버그 수정 완료). 3·4·5는 안 함.
